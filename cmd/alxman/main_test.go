...
const validManifest = `{
  "id": "alemonx-docker",
  "name": "Docker Compose 管理器",
  "version": "0.0.1",
  "runtime": "binary",
  "entry": {
    "darwin-arm64": "dist/alemonx-docker-darwin-arm64",
    "go": "runner"
  },
  "development": {
    "runtime": "go",
...
const fullManifest = `{
  ...
  "web": {"root": "web"},
  "statusActions": ["docker-status", "project-list"],
  "uploads": [{"action": "upload-compose", "maxBytes": 2097152}],
  ...
  "privilegedOperations": [
    {
      ...
      "runnerAction": "docker-install-approved",
      "planAction": "plan",
      "title": "授权安装 Docker",
    ...
    {
      "action": "docker-repair",
      "runnerAction": "docker-repair-approved",
      "useLatestAudit": true,
      "title": "撤销",
      "authorization": "native",
      "platforms": ["darwin", "linux", "windows"]
...
func TestValidateBadID(t *testing.T) {
	manifest := strings.Replace(validManifest, `"id": "alemonx-docker"`, `"id": "Bad_ID"`, 1)
...
func TestValidateUploadsRejectsDuplicatesAndBadLimits(t *testing.T) {
	duplicate := mutateManifest(t, fullManifest, func(m map[string]any) {
		m["uploads"] = []any{
			map[string]any{"action": "upload-compose", "maxBytes": 1024},
			map[string]any{"action": "upload-compose", "maxBytes": 1024},
		}
	})
	if code := validate(writeManifest(t, duplicate)); code != 1 {
		t.Fatalf("duplicate upload action returned %d, want 1", code)
	}
	badLimit := mutateManifest(t, fullManifest, func(m map[string]any) {
		m["uploads"] = []any{map[string]any{"action": "upload-compose", "maxBytes": -1}}
	})
	if code := validate(writeManifest(t, badLimit)); code != 1 {
		t.Fatalf("bad maxBytes returned %d, want 1", code)
	}
	tooLarge := mutateManifest(t, fullManifest, func(m map[string]any) {
		m["uploads"] = []any{map[string]any{"action": "upload-compose", "maxBytes": 128 * 1024 * 1024}}
	})
	if code := validate(writeManifest(t, tooLarge)); code != 1 {
		t.Fatalf("oversized maxBytes returned %d, want 1", code)
	}
}
