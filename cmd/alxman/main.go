...
func validateUploads(manifest map[string]any, errs *[]string) {
	uploads, present := manifest["uploads"]
	if !present || uploads == nil {
		return
	}
	items, ok := asSlice(uploads)
	if !ok {
		*errs = append(*errs, "uploads must be an array")
		return
	}
	seen := map[string]bool{}
	for _, raw := range items {
		upload, ok := asMap(raw)
		if !ok {
			*errs = append(*errs, "uploads entry must be an object")
			continue
		}
		action, _ := asString(upload["action"])
		action = strings.TrimSpace(action)
		if action == "" || len(action) > 96 || seen[action] {
			*errs = append(*errs, "setup plugin uploads require a unique action name")
		}
		if maxBytes, present := upload["maxBytes"]; present {
			if limit, ok := maxBytes.(float64); !ok || limit < 1 || limit > 64*1024*1024 {
				*errs = append(*errs, "setup plugin upload maxBytes must be between 1 and 64 MiB")
			}
		}
		if action != "" {
			seen[action] = true
